(() => {
  ...
  function upgradeComprehensive() {
    replaceText('.brand p', 'Unit 01～Unit 10', 'Unit 01～Unit 14');
    replaceText('.brand p', 'Unit 01～Unit 11', 'Unit 01～Unit 14');
    replaceText('.brand p', 'Unit 01～Unit 12', 'Unit 01～Unit 14');
    replaceText('.brand p', 'Unit 01～Unit 13', 'Unit 01～Unit 14');

    if (typeof normalizeWord === 'function' && !normalizeWord.__unit14Patched) {
      ...
      normalizeWord.__unit14Patched = true;
    }

    const choices = document.querySelector('#unitChoices');
    if (!choices) return;

    for (const no of [11, 12, 13, 14]) {
    ...
    saved = [...new Set((Array.isArray(saved) ? saved : []).map(Number))]
      .filter(no => Number.isInteger(no) && no >= 1 && no <= 14)
      .sort((a, b) => a - b);
    ...
    const applySaved = () => {
      try {
        ...
        console.warn('Unit 14 comprehensive upgrade:', error);
      }
    };
    applySaved();
  }
  ...
  function upgradeGrammar() {
    try {
      ...
      document.title = 'Unit 01-14 Exam Vocabulary Cloze';
      const h1 = document.querySelector('.brand h1');
      if (h1) h1.textContent = '1-14 課例句單字選擇題';
      const p = document.querySelector('.brand p');
      if (p) p.textContent = '整合 Unit 01 到 Unit 14。重新整理頁面時會先把題目洗牌；按「下一題」會依照本次洗牌後的順序出題。中文與解析作答後才顯示。';
      const loadingNode = document.querySelector('.loading');
      if (loadingNode) loadingNode.textContent = '正在載入 Unit 01-14 題庫...';
      ...
      console.warn('Unit 14 grammar upgrade:', error);
    }
  }
