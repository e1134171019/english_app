(() => {
  ...
  if (window.__comprehensiveUnit19UpgradeLoaded) return;
  window.__comprehensiveUnit19UpgradeLoaded = true;

  const TOTAL = 19;
  const STORAGE_KEY = 'vocab_selected_units_v3';
  const LEGACY_KEYS = ['vocab_selected_units_v2', 'vocab_selected_units_v1'];
  ...
  function renderSelector(units) {
    ...
    container.innerHTML = Array.from({ length: TOTAL }, (_, index) => index + 1).map(no => `
      <label class="unitToggle${units.includes(no) ? ' selected' : ''}${no === 19 ? ' unit19Featured' : ''}">
  ...
  function installQuickButtons() {
    ...
    if (!actions || actions.querySelector('#onlyUnit19')) return;

    const groups = [
      ['Unit 01–06', no => no >= 1 && no <= 6],
      ['Unit 07–12', no => no >= 7 && no <= 12],
      ['Unit 13–19', no => no >= 13 && no <= 19]
    ];
    ...
    const only19 = document.createElement('button');
    only19.type = 'button';
    only19.id = 'onlyUnit19';
    only19.className = 'smallBtn unit19Button';
    only19.textContent = '只選 Unit 19';
    only19.onclick = () => setChecks(no => no === 19);

    actions.prepend(...buttons, only19);
  }

  function installUnit19Link() {
    const head = document.querySelector('.unitHead');
    if (!head || head.querySelector('.unit19CourseLink')) return;
    const link = document.createElement('a');
    link.className = 'unit19CourseLink';
    link.href = '../unit19-vocab-lab/';
    link.textContent = '查看 Unit 19 教材';
    head.appendChild(link);
  }

  function installCopy() {
    const intro = document.querySelector('.brand p');
    if (intro) intro.textContent = '自選 Unit 01～Unit 19；套用後，所有測驗模式只會從已選課別出題。';
    const hint = document.querySelector('.unitHead p');
    if (hint) hint.textContent = '可單選或複選；第一次開啟新版綜合頁時，預設只練 Unit 19。';
  }
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .unit19Featured{box-shadow:inset 0 0 0 2px #2563eb}
      .unit19Button{background:#dbeafe;color:#1d4ed8}
      .unit19CourseLink{border-radius:13px;background:#2563eb;color:#fff;padding:10px 14px;text-decoration:none;font-weight:900;white-space:nowrap}
      @media(max-width:860px){.unitChoices{grid-template-columns:repeat(3,1fr)}.unitHead{align-items:center}}
      @media(max-width:520px){.unitChoices{grid-template-columns:repeat(2,1fr)}.unitHead{display:block}.unit19CourseLink{display:inline-block;margin-top:12px}.unitActions{grid-template-columns:1fr 1fr}.unitActions .primary{grid-column:1/-1}}
    `;
