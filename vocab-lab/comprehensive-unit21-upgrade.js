(() => {
  ...
  if (window.__comprehensiveUnit21UpgradeLoaded) return;
  window.__comprehensiveUnit21UpgradeLoaded = true;

  const TOTAL = 21;
  const STORAGE_KEY = 'vocab_selected_units_v5';
  const LEGACY_KEYS = ['vocab_selected_units_v4', 'vocab_selected_units_v3', 'vocab_selected_units_v2', 'vocab_selected_units_v1'];
  ...
  function renderSelector(units) {
    ...
    container.innerHTML = Array.from({ length: TOTAL }, (_, index) => index + 1).map(no => `
      <label class="unitToggle${units.includes(no) ? ' selected' : ''}${no === 21 ? ' unit21Featured' : ''}">
  ...
  function installQuickButtons() {
    ...
    if (!actions || actions.querySelector('#onlyUnit21')) return;

    const groups = [
      ['Unit 01–07', no => no >= 1 && no <= 7],
      ['Unit 08–14', no => no >= 8 && no <= 14],
      ['Unit 15–21', no => no >= 15 && no <= 21]
    ];
    ...
    const only21 = document.createElement('button');
    only21.type = 'button';
    only21.id = 'onlyUnit21';
    only21.className = 'smallBtn unit21Button';
    only21.textContent = '只選 Unit 21';
    only21.onclick = () => setChecks(no => no === 21);

    actions.prepend(...buttons, only21);
  }

  function installUnit21Link() {
    const head = document.querySelector('.unitHead');
    if (!head || head.querySelector('.unit21CourseLink')) return;
    const link = document.createElement('a');
    link.className = 'unit21CourseLink';
    link.href = '../unit21-vocab-lab/';
    link.textContent = '查看 Unit 21 教材';
    head.appendChild(link);
  }

  function installCopy() {
    const intro = document.querySelector('.brand p');
    if (intro) intro.textContent = '自選 Unit 01～Unit 21；套用後，所有測驗模式只會從已選課別出題。';
    const hint = document.querySelector('.unitHead p');
    if (hint) hint.textContent = '可單選或複選；第一次開啟新版綜合頁時，預設只練 Unit 21。';
  }

  function installStyles() {
    const style = document.createElement('style');
    style.textContent = `
      .unitChoices{grid-template-columns:repeat(7,minmax(0,1fr))}
      .unit21Featured{box-shadow:inset 0 0 0 2px #2563eb}
      .unit21Button{background:#dbeafe;color:#1d4ed8}
      .unit21CourseLink{border-radius:13px;background:#2563eb;color:#fff;padding:10px 14px;text-decoration:none;font-weight:900;white-space:nowrap}
      ...
      @media(max-width:520px){.unitChoices{grid-template-columns:repeat(2,1fr)}.unitHead{display:block}.unit21CourseLink{display:inline-block;margin-top:12px}.unitActions{grid-template-columns:1fr 1fr}.unitActions .primary{grid-column:1/-1}}
    `;
